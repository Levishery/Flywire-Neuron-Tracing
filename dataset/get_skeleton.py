...
csv_list = os.listdir(block_connector)
block_done = {}
if os.path.exists('block_done.npy'):
    block_done = np.load('block_done.npy', allow_pickle=True).item()
for f in csv_list:
    if not (f in block_done and block_done[f]):
        ...
        for i in tqdm(df.index):
            ...
            for id in missing:
                ...
                if len(mesh.vertices) > 15:
                    try:
                        fixed = sk.pre.fix_mesh(mesh, remove_disconnected=5, inplace=False)
                        skel = sk.skeletonize.by_wavefront(fixed, waves=1, step_size=2, progress=False)
        ...
        block_done[f] = True
        np.save('block_done.npy', block_done)
