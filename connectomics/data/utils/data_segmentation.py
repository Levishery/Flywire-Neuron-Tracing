...
from .local_shape_descriptor_gpu import get_local_shape_descriptors_gpu
...
def seg2LSD(label, topt):
    # lsd
    assert label.ndim in [2, 3], \
        'Undefined LSD for ndim=' + str(label.ndim)
    if USE_GPU==True:
        return get_local_shape_descriptors_gpu(label, sigma=(120, 120, 120), voxel_size=(6, 6, 30), downsample=2)
    else:
...
def seg_to_targets(label_orig: np.ndarray,
    ...
    for tid, topt in enumerate(topts):
        ...
        elif topt[0] == '8':  # local shape descriptor
            out[tid] = seg2LSD(label, topt)
