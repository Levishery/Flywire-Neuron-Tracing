...
with cp.cuda.Device(0):
    mempool.set_limit(size=700*1024**2)
limit = 700*1024**2

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def get_local_shape_descriptors_gpu(
        segmentation,
        sigma,
        voxel_size=None,
        roi=None,
...
class LsdExtractor(object):
    ...
    def __init__(self, sigma, mode='gaussian', downsample=1):
        ...
        self.coords = {}
        self.worker_id = torch.utils.data.get_worker_info()
        self.dev = cp.cuda.Device(0)
        if self.worker_id.id%2 == 1:
            try:
                self.dev = cp.cuda.Device(1)
                _ = self.dev.mem_info
            except:
                self.dev= cp.cuda.Device(0)
        ...
        with self.dev:
            ...
            if (sub_shape, sub_voxel_size) not in self.coords:
                ...
                try:
                    # 3d by default
                    grid = cp.meshgrid(
                            cp.arange(0, sub_shape[0]*sub_voxel_size[0], sub_voxel_size[0]),
                            cp.arange(0, sub_shape[1]*sub_voxel_size[1], sub_voxel_size[1]),
                            cp.arange(0, sub_shape[2]*sub_voxel_size[2], sub_voxel_size[2]),
                            indexing='ij')
                ...
                self.coords[(sub_shape, sub_voxel_size)] = cp.array(grid, dtype=cp.float32)

            coords = self.coords[(sub_shape, sub_voxel_size)]

            # for all labels
            logger.debug("GPU memory used: %s", mempool.used_bytes()/limit)
            for label in labels:
                ...
                mask = (cp.asarray(segmentation)==label).astype(cp.float32)
                ...
                sub_descriptor = cp.concatenate([
                    sub_mean_offset,
                    sub_variance,
                    sub_pearson,
                    sub_count[None,:]])
                del sub_mean_offset, sub_variance, sub_pearson, sub_count
                logger.debug("GPU memory used: %s", mempool.used_bytes() / limit)
                ...
                start = time.time()
                cp.multiply(descriptor, mask[roi_slices], descriptor)
            ...
            # clip outliers
            cp.clip(descriptors, 0.0, 1.0, out=descriptors)
        return descriptors
    ...
    def __get_stats(self, coords, mask, sigma_voxel, roi):
        ...
        if count_len == 3:
            ...
            pearson[0] /= cp.sqrt(variance[0]*variance[1])
            pearson[1] /= cp.sqrt(variance[0]*variance[2])
            pearson[2] /= cp.sqrt(variance[1]*variance[2])
            ...
            # normalize Pearson correlation coefficient
            variance[variance<1e-3] = 1e-3 # numerical stability
            pearson /= cp.sqrt(variance[0]*variance[1])
    ...
    def __make_sphere(self, radius):
        ...
        r2 = cp.arange(-radius, radius)**2
        dist2 = r2[:, None, None] + r2[:, None] + r2
        return (dist2 <= radius**2).astype(cp.float32)
    ...
    def __outer_product(self, array):
        ...
        k = array.shape[0]
        outer = cp.einsum('i...,j...->ij...', array, array)
