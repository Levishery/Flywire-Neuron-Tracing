...
class Compose(object):
    ...
    def smooth_edge(self, masks):
        ...
        # temp = smoothed_masks.copy()
        # for idx in np.unique(temp):
        #     if idx != 0:
        #         binary = (temp == idx).astype(np.uint8)
        #         for _ in range(2):
        #             binary = cv2.GaussianBlur(binary, ksize=(5,5), sigmaX=self.smooth_sigma, sigmaY = self.smooth_sigma)
        #             binary = (binary > self.smooth_threshold).astype(np.uint8)
        #         plt.imshow(temp[5])
        #         plt.show()
        #         temp[np.where(temp == idx)] = 0
        #         temp[np.where(binary == 1)] = idx
        # if DEBUG:
        #     print('smooth time: ', time.perf_counter() - self.start_time)
        # smoothed_masks = temp
    ...
    def __call__(self, sample, random_state=np.random.RandomState()):
        ...
        for tid, t in enumerate(reversed(self.transforms)):
            if ran[tid] < t.p:
                ...
                if DEBUG:
                    print('augmentation time: ', tid, time.perf_counter() - self.start_time)
