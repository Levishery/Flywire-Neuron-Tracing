...
class Trainer(object):
    ...
    def test_patch(self, mode: str, rank=None):
        ...
        img_dir = 'fafbv14_h5'
        embedding_path = patch_path.replace(img_dir, embedding_dir)
        embedding_z_path = img_z_path.replace(img_dir, embedding_dir)
        # write h5 dataset
        ...
        with torch.no_grad():
            for i, sample in enumerate(self.dataloader):
                ...
                pos, volume = sample.pos, sample.out_input.unsqueeze(dim=1)
                volume = volume.to(self.device, non_blocking=True)
