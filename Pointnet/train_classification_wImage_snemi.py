...
from data_utils.SNEMI3DDataLoader import SNEMI3DDataLoader
...
def test(model, loader, summary_writer, criterion, step):
    ...
    return recall, accuracy, loss_vis/j
...
def main(args):
    ...
    data_path_train = '/braindat/lab/liusl/aaai24/snemi3d/pc-train-2048'
    data_path_test = '/braindat/lab/liusl/aaai24/snemi3d/pc-test-2048'
    positive_ratio = -1 # -1 means sample randomly
    alpha = 0.7 # -1 means no focal loss

    train_dataset = SNEMI3DDataLoader(root=data_path_train, split='train', process_data=args.process_data, use_image_feature=args.image_feature, npoints=args.num_point, positive_ratio=positive_ratio)
    test_dataset = SNEMI3DDataLoader(root=data_path_test, split='test', process_data=args.process_data, use_image_feature=args.image_feature, npoints=args.num_point)
    ...
    criterion = model.get_loss(alpha=alpha)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),
            ...
            optimizer.step()
            global_step += 1
            if batch_id % 30 == 0 and batch_id > 10:
                recall = TP_total / (TP_total + FN_total)
                accuracy = TP_total / (TP_total + FP_total)
        ...
        savepath = str(checkpoints_dir) + '/epoch%s.pth' % global_epoch
        log_string('Saving at %s' % savepath)
        state = {
            'epoch': best_epoch,
            'Recall': recall,
