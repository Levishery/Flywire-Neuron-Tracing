...
def test(model, loader, summary_writer, criterion, step, prediction_csv):
    FP_total = 0
    TP_total = 0
    FN_total = 0
    mean_correct = []
...
def main(args):
    ...
    data_path = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048'
    ...
    '''TESTING'''
    prefix = 'test_2048'
    block_root = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048/block_names'
    if args.image_feature is not None:
        result_path = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048/result_' + args.image_feature[:-1] + prefix + '.csv'
        prediction_path = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048/prediction_' + args.image_feature[:-1] + prefix
    else:
        result_path = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048/result_' + 'pc' + prefix + '.csv'
        prediction_path = '/braindat/lab/liusl/flywire/block_data/v2/point_cloud/test_fps_2048/prediction_' + 'pc' + prefix
    if not os.path.exists(prediction_path):
        os.mkdir(prediction_path)

    test_names = os.listdir(block_root)
    for test_block in test_names:
        prediction_csv = os.path.join(prediction_path, test_block.split('.')[0] + '.csv')
        if not os.path.exists(prediction_csv):
            test_dataset = FlyTracingDataLoader(root=data_path, split='test', process_data=args.process_data, use_image_feature=args.image_feature, test_path=os.path.join(block_root, test_block), daiyi_datapath=False, return_name=True, npoints=args.num_point)
            testDataLoader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                                                         num_workers=8)
            with torch.no_grad():
                start = time.perf_counter()
                recall, acc, test_loss = test(classifier, testDataLoader, summary_writer=None, criterion=None, step=0, prediction_csv=prediction_csv)
